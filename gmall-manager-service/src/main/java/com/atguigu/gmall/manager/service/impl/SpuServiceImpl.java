...
public class SpuServiceImpl implements SpuService {
    ...
    public void saveSpuInfo(PmsProductInfo pmsProductInfo) {
        ...
        String pro_id = pmsProductInfo.getId();

        // 保存SpuSaleAttr
        List<PmsProductSaleAttr> spuSaleAttrList = pmsProductInfo.getSpuSaleAttrList();
        for (PmsProductSaleAttr pmsProductSaleAttr : spuSaleAttrList) {
            pmsProductSaleAttr.setProductId(pro_id);
            pmsProductSaleAttrMapper.insertSelective(pmsProductSaleAttr);

            // 保存SpuSaleAttrValue
            List<PmsProductSaleAttrValue> spuSaleAttrValueList = pmsProductSaleAttr.getSpuSaleAttrValueList();
            for (PmsProductSaleAttrValue pmsProductSaleAttrValue : spuSaleAttrValueList) {
                pmsProductSaleAttrValue.setProductId(pro_id);
        ...
        for (PmsProductImage pmsProductImage : spuImageList) {
            pmsProductImage.setProductId(pro_id);
...
//        }
        ...
        for (PmsProductSaleAttr productSaleAttr : pmsProductSaleAttrs) {
            ...
            pmsProductSaleAttrValue1.setProductId(spuId);
            pmsProductSaleAttrValue1.setSaleAttrId(productSaleAttr.getSaleAttrId());
            List<PmsProductSaleAttrValue> pmsProductSaleAttrValues = pmsProductSaleAttrValueMapper.select(pmsProductSaleAttrValue1);
...
//        System.out.println("移除数据源 [" + DynamicDataSourceContextHolder.getDataSourceKey());
    ...
    public List<PmsProductSaleAttr> getSpuSaleAttrListCheckBySku(String spuId, String skuId) {
        List<PmsProductSaleAttr> PmsProductSaleAttr = pmsProductSaleAttrMapper.selectSpuSaleAttrListCheckBySku(spuId, skuId);
